...
class CalculatorToolProvider:
    async def list_tools(self) -> list[ToolDefinition]:
        return [ToolDefinition(_CALCULATE, "Calculate an arithmetic expression locally. Always use this tool for mathematical operations; never do mental math.", {"type": "object", "properties": {"expression": {"type": "string", "minLength": 1, "maxLength": 256}}, "required": ["expression"], "additionalProperties": False})]
    ...
    async def call_tool(self, name: str, arguments: dict[str, Any]) -> ToolResult:
        ...
        if not isinstance(expression, str):
            return ToolResult([{"type": "text", "text": "expression must be a string"}], True)
        try:
            result = _evaluate(ast.parse(expression, mode="eval").body)
        except (SyntaxError, ValueError, ZeroDivisionError, OverflowError) as err:
            return ToolResult([{"type": "text", "text": f"Calculation failed: {err}"}], True)
